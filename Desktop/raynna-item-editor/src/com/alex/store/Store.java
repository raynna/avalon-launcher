...
public final class Store {
	...
	private RandomAccessFile datam;
	...
	public Store(String path, boolean newProtocol, int[][] keys) throws IOException {
		...
		File fdm = new File(path, "main_file_cache.dat2m");
		if (fdm.exists()) {
			datam = new RandomAccessFile(path + "main_file_cache.dat2m", "rw");
		}
		...
		for (int id = 0; id < idxsCount; id++) {
			Index index = new Index(index255, new MainFile(id, id == 40 && datam != null ? datam : data, new RandomAccessFile(path + "main_file_cache.idx" + id, "rw"), newProtocol), keys == null ? null : keys[id]);
