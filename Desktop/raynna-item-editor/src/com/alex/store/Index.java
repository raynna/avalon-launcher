...
public final class Index {
	...
	protected Index(MainFile index255, MainFile mainFile, int[] keys) {
		...
		crc = CRC32HGenerator.getCrc(archiveData);
	...
	public boolean putArchive(int groupId, byte[] data, int name, Integer[] fileNames, boolean rewriteTable, boolean resetCache) {
		if (data == null) {
			return false;
		}
		if (!archiveExists(groupId)) {
			table.addEmptyArchiveReference(groupId);
		}
		ArchiveReference reference = table.getArchives()[groupId];
		reference.setRevision(data[data.length - 2] << 8 | data[data.length]);
		reference.updateHeader(data, name, fileNames);
		int revision = reference.getRevision();
		data[data.length - 2] = (byte) (revision >> 8);
		data[data.length - 1] = (byte) revision;
		if (!mainFile.putArchiveData(groupId, data)) {
			return false;
		}
	...
	public boolean putArchive(int id, int archiveId, int newArchiveId, Store originalStore, boolean rewriteTable, boolean resetCache) {
		try {
			Index originalIndex = originalStore.getIndexes()[id];
			...
			if (!archiveExists(newArchiveId)) {
				table.addEmptyArchiveReference(newArchiveId);
			}
			ArchiveReference reference = table.getArchives()[newArchiveId];
			reference.updateRevision();
			ArchiveReference originalReference = originalIndex.table.getArchives()[archiveId];
			reference.copyHeader(originalReference, data, true);
			int revision = reference.getRevision();
			data[data.length - 2] = (byte) (revision >> 8);
			data[data.length - 1] = (byte) revision;
			if (!mainFile.putArchiveData(newArchiveId, data)) {
				return false;
			}
			if (rewriteTable && !rewriteTable()) {
				return false;
	...
	public boolean putFileNoRewriteTable(int archiveId, int fileId, byte[] data) {
		return this.putFile(archiveId, fileId, 2, data, (int[])null, false, false, -1, -1);
	}
	...
	public boolean removeFile(int archiveId, int fileId, int compression, int[] keys) {
		...
		reference.setCrc(CRC32HGenerator.getCrc(closedArchive, 0, closedArchive.length - 2));
	...
	public boolean exists(int id) {
		if (table.getArchives().length == 1) {
			return fileExists(0, id);
		}
		if (!archiveExists(id)) {
			return false;
		}
		if (table.getArchives()[id].getFiles().length == 1) {
	...
	public int[] listFiles(int groupId) {
		if (!archiveExists(groupId)) {
			return null;
		}
		return table.getArchives()[groupId].getValidFileIds();
	}
}
